...
const CartProvider = ({ children }) => {
  ...
  const { user } = useAuth();
console.log(axiosSecure,user?.email);
  // Cart state
  ...
  // };
 const handleAddToCart = (petData) => {
    const cartItem={
      petId:petData?._id,
      petName:petData?.petName,
      petPic:petData?.petPic,
      petCategory:petData?.petCategory
    }
    const isItemInCart = cart.find(item => item.petId === cartItem.petId);

    if (!isItemInCart) {
      setCart([...cart, cartItem]); // নতুন পণ্য যোগ করুন
      console.log('Cart updated:', [...cart, cartItem]);
    } else {
      Swal.fire({
        title: "Already in Cart!",
        text: `${cartItem.petName} Item is already in your cart.`,
        icon: "info",
