...
const ProfileHome = () => {
  ...
  const axiosSecure=useAxiosSecure()
    const { data: totalOrders = 0, isLoading,  } = useQuery({
    queryKey: ["total-orders", user?.email],
    queryFn: async () => {
      const res = await axiosSecure.get(`/users/orders/count?email=${user?.email}`);
      return res.data.totalOrders;
    },
  });
  if (isLoading) return <Loading/>
  console.log(totalOrders,'This is total order ');
  return (
    <div className=" relative">
      ...
      <div className="container mx-auto">
        {/* TOTAL ORDERS CARD*/}
        <div className="relative w-full max-w-xs sm:max-w-lg  rounded-xl p-5 lg:p-9 overflow-hidden bg-gradient-to-r from-green-600 to-emerald-500 text-white shadow-2xl">
          ...
          <div className="relative flex flex-col  justify-between h-full">
            ...
            <div className="flex flex-col justify-between items-start">
              ...
              <h2 className="text-xl md:text-2xl lg:text-3xl font-medium  lg:font-bold tracking-wide">Total Orders</h2>
              {/* User Icon Badge  */}
              ...
              <div className="flex-1 flex items-center pt-2">
                <p className="text-2xl md:text-3xl lg:text-6xl font-bold lg:font-extrabold ">{totalOrders}</p>
              </div>
              ...
              <div className="flex flex-col  justify-end mt-2 space-y-2 lg:space-y-3">
                ...
                <Link to={'/all-pets'}>
                <button
                  className="px-3 py-1 lg:py-2 cursor-pointer rounded-full text-white/90 hover:bg-green-700 transition flex items-center bg-light-accent/60"
